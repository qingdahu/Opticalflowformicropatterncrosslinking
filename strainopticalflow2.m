...
for i=1:size(Ux,1)
    for j=1:size(Ux,2)
        F = squeeze(Hdisplacementgradient(i,j,:,:)) + eye(2);
        alignmentdistribution = [1:360];
        slicesize = alignmentdistribution(2)-alignmentdistribution(1);
        circlesize = 0.4;
        numofslices = ceil(36/2);
        
        testvectors = [cosd(alignmentdistribution)', sind(alignmentdistribution)'];
        testvectorsprime = testvectors * F;
        alignmentdistribution = atand(testvectorsprime(:,2)./testvectorsprime(:,1));

        for slice = 1:numofslices
            anglemax = 180/numofslices * ( slice - 0.5 );
            anglemin = 180/numofslices * ( slice - 1.5 );

            anglecount = sum(   ( alignmentdistribution<anglemax &  alignmentdistribution>anglemin ) | ( alignmentdistribution<(anglemax-180) &  alignmentdistribution>(anglemin-180) )    );


            patchx = [ i,i+circlesize*cosd(anglemax),i+circlesize*cosd(anglemin)];
            patchy = [ j,j-circlesize*sind(anglemax),j-circlesize*sind(anglemin)];
            color = [0.5 , 0.5 , 0.5]* ((anglecount / (length(alignmentdistribution)/numofslices) -1)*4 +1       ) ;
            if anglecount>(length(alignmentdistribution)/numofslices)*1.2
                color = [1 1 1];
            elseif anglecount<(length(alignmentdistribution)/numofslices)*0.8
                color = [0 0 0];
            end
            patch(patchx,patchy, color , 'EdgeColor', 'none')
...
for i=1:size(Ux,1)
    for j=1:size(Ux,2)
        F = squeeze(Hdisplacementgradient(i,j,:,:)) + (squeeze(Hdisplacementgradient(i,j,:,:))+squeeze(Hdisplacementgradient(i,j,:,:))')*5 + eye(2);                                         % do we only want to scale up the stretching and not the rotation?
        alignmentdistribution = [1:360];
        slicesize = alignmentdistribution(2)-alignmentdistribution(1);
        circlesize = 0.3;
        numofslices = ceil(36/2);
        ...
        testvectors = [cosd(alignmentdistribution)', sind(alignmentdistribution)'];
        testvectorsprime = testvectors.*circlesize * F;
        testvectorsprimex = i + testvectorsprime(:,1);
        testvectorsprimey = j - testvectorsprime(:,2);
        fill(testvectorsprimex,testvectorsprimey,[0.5 0.5 0.5])
...
% 
% 
% temp1 = alignmentdistributions([10:20,60:70],20:60,:);
% figure
% subplot(2,2,1)
% histogram(temp1(:) )
% temp1 = alignmentdistributions(30:50, 20:60,:);
% subplot(2,2,2)
% histogram( temp1(:))